import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:ptit_dms_flutter/core/error/app_exception.dart';
import 'package:ptit_dms_flutter/domain/entities/research_post_acceptance_report.dart';
import 'package:ptit_dms_flutter/domain/entities/research_post_acceptance_report_request.dart';
import 'package:ptit_dms_flutter/domain/repositories/research_post_acceptance_report_repository.dart';

import 'research_post_acceptance_report_event.dart';
import 'research_post_acceptance_report_state.dart';

export 'research_post_acceptance_report_event.dart';
export 'research_post_acceptance_report_state.dart';

class ResearchPostAcceptanceReportBloc
    extends
        Bloc<
          ResearchPostAcceptanceReportEvent,
          ResearchPostAcceptanceReportState
        > {
  ResearchPostAcceptanceReportBloc({
    required ResearchPostAcceptanceReportRepository repository,
  }) : _repository = repository,
       super(const ResearchPostAcceptanceReportState()) {
    on<ResearchPostAcceptanceReportStarted>(_onStarted);
    on<ResearchPostAcceptanceReportRefreshed>(_onRefreshed);
    on<ResearchPostAcceptanceReportUploaded>(_onUploaded);
    on<ResearchPostAcceptanceReportUploadStateCleared>(_onUploadStateCleared);
    on<ResearchPostAcceptanceReportFileDownloaded>(_onFileDownloaded);
    on<ResearchPostAcceptanceReportDownloadStateCleared>(
      _onDownloadStateCleared,
    );
  }

  final ResearchPostAcceptanceReportRepository _repository;
  int _loadGeneration = 0;

  Future<void> _onStarted(
    ResearchPostAcceptanceReportStarted event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) async {
    final researchId = event.researchId.trim();
    final yearId = event.yearId.trim();
    final validationMessage = _validateIdentifiers(
      researchId: researchId,
      yearId: yearId,
    );

    if (validationMessage != null) {
      _loadGeneration++;
      emit(
        state.copyWith(
          loadStatus: ResearchPostAcceptanceReportLoadStatus.failure,
          researchId: researchId,
          yearId: yearId,
          reports: const [],
          loadErrorMessage: validationMessage,
        ),
      );
      return;
    }

    await _load(
      emit,
      researchId: researchId,
      yearId: yearId,
      clearReports: researchId != state.researchId || yearId != state.yearId,
    );
  }

  Future<void> _onRefreshed(
    ResearchPostAcceptanceReportRefreshed event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) async {
    final validationMessage = _validateIdentifiers(
      researchId: state.researchId,
      yearId: state.yearId,
    );
    if (validationMessage != null) {
      emit(
        state.copyWith(
          loadStatus: ResearchPostAcceptanceReportLoadStatus.failure,
          loadErrorMessage: validationMessage,
        ),
      );
      return;
    }

    await _load(
      emit,
      researchId: state.researchId,
      yearId: state.yearId,
      clearReports: false,
    );
  }

  Future<void> _load(
    Emitter<ResearchPostAcceptanceReportState> emit, {
    required String researchId,
    required String yearId,
    required bool clearReports,
  }) async {
    final generation = ++_loadGeneration;
    emit(
      state.copyWith(
        loadStatus: ResearchPostAcceptanceReportLoadStatus.loading,
        researchId: researchId,
        yearId: yearId,
        reports: clearReports ? const [] : state.reports,
        loadErrorMessage: null,
      ),
    );

    try {
      final reports = await _repository.getReports(
        researchId: researchId,
        yearId: yearId,
      );
      if (!_canEmitLoad(emit, generation, researchId, yearId)) return;

      emit(
        state.copyWith(
          loadStatus: ResearchPostAcceptanceReportLoadStatus.success,
          reports: List<ResearchPostAcceptanceReport>.unmodifiable(reports),
          loadErrorMessage: null,
        ),
      );
    } on AppException catch (error) {
      if (!_canEmitLoad(emit, generation, researchId, yearId)) return;
      emit(
        state.copyWith(
          loadStatus: ResearchPostAcceptanceReportLoadStatus.failure,
          loadErrorMessage: error.message,
        ),
      );
    } catch (_) {
      if (!_canEmitLoad(emit, generation, researchId, yearId)) return;
      emit(
        state.copyWith(
          loadStatus: ResearchPostAcceptanceReportLoadStatus.failure,
          loadErrorMessage: 'Không thể tải báo cáo sau nghiệm thu.',
        ),
      );
    }
  }

  Future<void> _onUploaded(
    ResearchPostAcceptanceReportUploaded event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) async {
    final validationMessage = _validateRequest(event.request);
    if (validationMessage != null) {
      emit(
        state.copyWith(
          uploadStatus: ResearchPostAcceptanceReportUploadStatus.failure,
          uploadProgress: 0,
          uploadMessage: validationMessage,
        ),
      );
      return;
    }

    final researchId = event.request.researchId.trim();
    final yearId = event.request.yearId.trim();
    emit(
      state.copyWith(
        uploadStatus: ResearchPostAcceptanceReportUploadStatus.uploading,
        uploadProgress: 0,
        uploadMessage: null,
      ),
    );

    var latestProgress = 0.0;
    try {
      await _repository.uploadReport(
        request: event.request,
        onSendProgress: (sentBytes, totalBytes) {
          if (emit.isDone || isClosed || totalBytes <= 0 || sentBytes < 0) {
            return;
          }
          final progress = (sentBytes / totalBytes).clamp(0.0, 1.0);
          if (progress <= latestProgress) return;
          latestProgress = progress;
          emit(state.copyWith(uploadProgress: progress));
        },
      );
      if (emit.isDone || isClosed) return;

      List<ResearchPostAcceptanceReport>? latestReports;
      String? reloadErrorMessage;
      try {
        latestReports = await _repository.getReports(
          researchId: researchId,
          yearId: yearId,
        );
      } on AppException catch (error) {
        reloadErrorMessage = error.message;
      } catch (_) {
        reloadErrorMessage = 'Không thể tải lại lịch sử báo cáo.';
      }
      if (emit.isDone || isClosed) return;

      final isCurrentContext =
          state.researchId == researchId && state.yearId == yearId;
      emit(
        state.copyWith(
          loadStatus: isCurrentContext && latestReports != null
              ? ResearchPostAcceptanceReportLoadStatus.success
              : state.loadStatus,
          uploadStatus: ResearchPostAcceptanceReportUploadStatus.success,
          reports: isCurrentContext && latestReports != null
              ? List<ResearchPostAcceptanceReport>.unmodifiable(latestReports)
              : state.reports,
          uploadProgress: 1,
          loadErrorMessage: isCurrentContext && reloadErrorMessage != null
              ? reloadErrorMessage
              : state.loadErrorMessage,
          uploadMessage: reloadErrorMessage == null
              ? 'Nộp báo cáo sau nghiệm thu thành công.'
              : 'Nộp báo cáo thành công nhưng không thể tải lại lịch sử.',
        ),
      );
    } on AppException catch (error) {
      if (emit.isDone || isClosed) return;
      emit(
        state.copyWith(
          uploadStatus: ResearchPostAcceptanceReportUploadStatus.failure,
          uploadMessage: error.message,
        ),
      );
    } catch (_) {
      if (emit.isDone || isClosed) return;
      emit(
        state.copyWith(
          uploadStatus: ResearchPostAcceptanceReportUploadStatus.failure,
          uploadMessage: 'Không thể nộp báo cáo sau nghiệm thu.',
        ),
      );
    }
  }

  Future<void> _onFileDownloaded(
    ResearchPostAcceptanceReportFileDownloaded event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) async {
    final fileKey = event.fileKey.trim();
    final yearId = (event.yearId ?? state.yearId).trim();
    final validationMessage = _validateDownload(
      fileKey: fileKey,
      yearId: yearId,
    );
    if (validationMessage != null) {
      emit(
        state.copyWith(
          downloadStatus: ResearchPostAcceptanceReportDownloadStatus.failure,
          downloadingFileKey: null,
          downloadedFile: null,
          downloadMessage: validationMessage,
        ),
      );
      return;
    }

    emit(
      state.copyWith(
        downloadStatus: ResearchPostAcceptanceReportDownloadStatus.downloading,
        downloadingFileKey: fileKey,
        downloadedFile: null,
        downloadMessage: null,
      ),
    );

    try {
      final downloadedFile = await _repository.downloadFile(
        fileKey: fileKey,
        yearId: yearId,
      );
      if (emit.isDone || isClosed) return;

      emit(
        state.copyWith(
          downloadStatus: ResearchPostAcceptanceReportDownloadStatus.success,
          downloadingFileKey: null,
          downloadedFile: downloadedFile,
          downloadMessage: 'Tải file báo cáo thành công.',
        ),
      );
    } on AppException catch (error) {
      if (emit.isDone || isClosed) return;
      emit(
        state.copyWith(
          downloadStatus: ResearchPostAcceptanceReportDownloadStatus.failure,
          downloadingFileKey: null,
          downloadedFile: null,
          downloadMessage: error.message,
        ),
      );
    } catch (_) {
      if (emit.isDone || isClosed) return;
      emit(
        state.copyWith(
          downloadStatus: ResearchPostAcceptanceReportDownloadStatus.failure,
          downloadingFileKey: null,
          downloadedFile: null,
          downloadMessage: 'Không thể tải file báo cáo sau nghiệm thu.',
        ),
      );
    }
  }

  String? _validateIdentifiers({
    required String researchId,
    required String yearId,
  }) {
    if (researchId.isEmpty) return 'Thiếu mã đề tài nghiên cứu.';
    if (yearId.isEmpty) return 'Thiếu năm học.';
    return null;
  }

  String? _validateRequest(ResearchPostAcceptanceReportRequest request) {
    try {
      request.validate();
      return null;
    } on FormatException catch (error) {
      return error.message;
    } catch (_) {
      return 'Thông tin hoặc file báo cáo không hợp lệ.';
    }
  }

  String? _validateDownload({required String fileKey, required String yearId}) {
    if (fileKey.isEmpty) return 'Thiếu thông tin file cần tải.';
    if (yearId.isEmpty) return 'Thiếu năm học.';
    return null;
  }

  void _onUploadStateCleared(
    ResearchPostAcceptanceReportUploadStateCleared event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) {
    emit(
      state.copyWith(
        uploadStatus: ResearchPostAcceptanceReportUploadStatus.initial,
        uploadProgress: 0,
        uploadMessage: null,
      ),
    );
  }

  void _onDownloadStateCleared(
    ResearchPostAcceptanceReportDownloadStateCleared event,
    Emitter<ResearchPostAcceptanceReportState> emit,
  ) {
    emit(
      state.copyWith(
        downloadStatus: ResearchPostAcceptanceReportDownloadStatus.initial,
        downloadingFileKey: null,
        downloadedFile: null,
        downloadMessage: null,
      ),
    );
  }

  bool _canEmitLoad(
    Emitter<ResearchPostAcceptanceReportState> emit,
    int generation,
    String researchId,
    String yearId,
  ) {
    return generation == _loadGeneration &&
        state.researchId == researchId &&
        state.yearId == yearId &&
        !emit.isDone &&
        !isClosed;
  }
}
